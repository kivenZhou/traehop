...
| | TraeHop | 常见「换号工具」 |
|---|---------|----------------|
| 数据存储 | 100% 本地，无远程服务器 | 部分工具含云端或不可审计闭源 |
| 切换方式 | 仅手动，由你决定何时切换 | 部分支持额度耗尽自动轮换 |
| 代码 | MIT 开源，可自行审查 | 多为闭源 |
| 平台 | macOS（Intel / Apple Silicon）+ Windows | 多数仅 Windows |
| 登录方式 | 浏览器登录（含 2FA）、Token、从 IDE 导入 | 通常仅 Token |
...
- 所有 Token、Cookie 保存在本机 [`~/Library/Application Support/traehop/`（macOS）或 `%APPDATA%\traehop\`（Windows）](https://github.com/kivenZhou/traehop#数据位置)，不上传远程
- 使用本工具**可能违反 Trae 用户协议**，封号等风险由您自行承担
...
```
启动 → 同意须知 → 设置里配置 Trae 路径 → 添加账号 → 点击「切换」
```

**添加账号** — 三选一：

1. **浏览器登录**（推荐）：弹出窗口登录 trae.ai，支持 2FA，成功后自动添加
2. **粘贴 Token**：F12 → Network → 筛选 `GetUserToken` → 复制响应中的 Token
3. **从 Trae 导入**：在 IDE 中登录目标账号后，一键导入当前会话
...
**备份与迁移**：账号页导出 JSON（可设密码加密），换机或团队共享配置时用导入功能恢复。
...
凭证默认以明文存于本地 electron-store。建议启用加密导出，并保持 OS 账户安全。
...
**切换后 Trae 没有自动打开？**
检查设置中的 Trae 路径是否正确；macOS 需确认已授予必要权限。

**Token 无效？**
检查是否有多余空格、是否已过期。优先用浏览器登录。

**切换后仍显示旧账号？**
手动关闭 Trae，尝试「重置环境后切换」，或在「清理」页执行环境清理后再切换。

**支持 Linux 吗？**
不支持。Trae IDE 官方仅支持 Windows 和 macOS。

**和其他工具有何不同？**
TraeHop 定位是合规的本地会话管理：开源可审计、纯本地、仅手动切换、双平台、支持浏览器登录与加密备份。详见上文对比表。
